...
namespace ME.ECS.Collections {
    ...
    public sealed class CCDictionary<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue> where TValue : IComparable<TValue>
    {
        ...
        /// </summary>
        public CCDictionary() : this(DefaultConcurrencyLevel, DEFAULT_CAPACITY, true, EqualityComparer<TKey>.Default) { }
        ...
        /// 0.</exception>
        public CCDictionary(int concurrencyLevel, int capacity) : this(concurrencyLevel, capacity, false, EqualityComparer<TKey>.Default) { }
        ...
        public CCDictionary(IEnumerable<KeyValuePair<TKey, TValue>> collection) : this(collection, EqualityComparer<TKey>.Default) { }
        ...
        /// (Nothing in Visual Basic).</exception>
        public CCDictionary(IEqualityComparer<TKey> comparer) : this(DefaultConcurrencyLevel, DEFAULT_CAPACITY, true, comparer) { }
        ...
        /// </exception>
        public CCDictionary(IEnumerable<KeyValuePair<TKey, TValue>> collection, IEqualityComparer<TKey> comparer)
            : this(comparer)
        {
        ...
        /// <exception cref="T:System.ArgumentNullException"><paramref name="comparer"/> is a null reference
        /// (Nothing in Visual Basic).</exception>
        public CCDictionary(int concurrencyLevel, int capacity, IEqualityComparer<TKey> comparer)
            : this(concurrencyLevel, capacity, false, comparer)
        {
        }
 
        internal CCDictionary(int concurrencyLevel, int capacity, bool growLockArray, IEqualityComparer<TKey> comparer)
        {
            ...
            if (comparer == null) comparer = EqualityComparer<TKey>.Default;//throw new ArgumentNullException("comparer");
        ...
        public void Clear()
        {
            ...
            try
            {
                ...
                System.Array.Clear(m_tables.m_buckets, 0, this.m_tables.m_buckets.Length);
                System.Array.Clear(m_tables.m_countPerLock, 0, this.m_tables.m_countPerLock.Length);
                
                //m_tables = newTables;
                m_budget = Math.Max(1, this.m_tables.m_buckets.Length / m_tables.m_locks.Length);
            }
            finally
            {
        ...
        private bool TryAddInternal(TKey key, TValue value, bool updateIfExists, bool acquireLock, out TValue resultingValue)
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    vNode.m_next = tables.m_buckets[bucketNo];
                    Volatile.Write<Node>(ref tables.m_buckets[bucketNo], vNode);
                    checked
                    {
                        tables.m_countPerLock[lockNo]++;
                    }
        ...
        {
            return ((CCDictionary<TKey, TValue>)this).GetEnumerator();
        }
        ...
        bool IDictionary.Contains(object key)
        {
            ...
            return (key is TKey) && ((CCDictionary<TKey, TValue>)this).ContainsKey((TKey)key);
        }
        ...
        object IDictionary.this[object key]
        {
            ...
            set
            {
                ...
                ((CCDictionary<TKey, TValue>)this)[(TKey)key] = (TValue)value;
            }
        }
            ...
            try
            {
                ...
                for (int i = 0; i < tables.m_buckets.Length; i++)
                {
                    ...
                    while (current != null)
                    {
                        ...
                        vNode.m_next = newBuckets[newBucketNo];
                        newBuckets[newBucketNo] = vNode;
        ...
        /// </summary>
        internal static int DefaultConcurrencyLevel
        {
        ...
        private void QuickSort(TValue[] array, int start, int end) {
            if (start >= end) {
                return;
            }
        ...
        public BufferArray<TValue> GetSortedValues() {
            ...
            try
            {
                ...
                var values = PoolArray<TValue>.Spawn(count);
                var k = 0;
                for (int i = 0; i < m_tables.m_buckets.Length; i++)
                {
                    Node current = m_tables.m_buckets[i];
                ...
                this.QuickSort(values.arr, 0, values.Length - 1);
                //System.Array.Sort(values.arr, 0, values.Length);
                return values;
            }
            finally
            {
        ...
        private class DictionaryEnumerator : IDictionaryEnumerator
        {
            ...
            internal DictionaryEnumerator(CCDictionary<TKey, TValue> dictionary)
            {
